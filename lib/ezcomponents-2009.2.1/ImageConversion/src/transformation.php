...
class ezcImageTransformation
{
    ...
    public function transform( $fileIn, $fileOut )
    {
        ...
        //Интересно зачем тут точка подставлялась перед именем файла?
        $fileTmp = tempnam( dirname( $fileOut ) . DIRECTORY_SEPARATOR,  basename( $fileOut ) );
        if(!copy( $fileIn, $fileTmp ))
        {
            throw new ezcBaseFileIoException( $fileTmp, ezcBaseFileException::WRITE );
        }
        ...
        try
        {
            ...
            //Виктор: правка исходников ezcomponents(OSP-1405)
        ...
        {
            // Clean up
            // Александр М.: правка исходников ezcomponents(OSP-2232)
            // Ошибка при удалении временного файла (которого нет) при неудаче определить его MIME тип
