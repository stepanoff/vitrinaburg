...
class ezcImageAnalyzerImagemagickHandler extends ezcImageAnalyzerHandler
{
    ...
    public function analyzeImage( $file )
    {
		...
		$format = ( ezcBaseFeatures::os() === 'Windows' ? '"[%m|%b|%w|%h|%k|%r|%c]RANDOMSTRING*RANDOMSTRING"' : escapeshellarg( '[%m|%b|%w|%h|%k|%r|%c]RANDOMSTRING*RANDOMSTRING' ) );
        $command = '-format ' . $format . ' ' . escapeshellarg( $file );
        // Execute ImageMagick
        ...
        $rawDataArr = explode( 'RANDOMSTRING*RANDOMSTRING', $outputString );
    ...
    protected function analyzeExif( ezcImageAnalyzerData $data, $file )
    {
        ...
        // Retreive exif data
        $command = (ezcBaseFeatures::os() === 'Windows') ? '-format ' . '"%[EXIF:*]"'  . ' ' .$file  : '-format ' . escapeshellarg( "%[EXIF:*]"  ) . ' ' . escapeshellarg( $file );
        ...
        //Yii::log(CVarDumper::dumpAsString($command));
        //Yii::log(CVarDumper::dumpAsString($dataArr));
        // Assign data to tags
        $exifArr = array();
        foreach ( $tagMap as $section => $tags )
        {
            ...
            {
                $tagIndex = 'exif:'.$tag;
                if ( isset( $dataArr[$tagIndex] ) )
                {
                    // Correct types
                    switch ( true )
                    {
                        case ( ctype_digit( $dataArr[$tagIndex] ) && stripos( $tag, "version" ) === false ):
                            $exifArr[$section][$tag] = (int)$dataArr[$tagIndex];
                            break;
                        case ( is_numeric( $dataArr[$tagIndex] ) && stripos( $tag, "version" ) === false ):
                            $exifArr[$section][$tag] = (float)$dataArr[$tagIndex];
                            break;
                        default:
                            $exifArr[$section][$tag] = $dataArr[$tagIndex];
                            break;
                    }
                }
            }
        ...
        //Yii::log(CVarDumper::dumpAsString($exifArr));
