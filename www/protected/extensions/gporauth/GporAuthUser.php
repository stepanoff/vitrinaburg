<?php
class GporAuthUser extends CApplicationComponent implements IWebUser
{
     ...
     */
    public $dbDriver='RedisGporAuthDbDriver';
    ...
    /*
     * одноразовый токен для отправки клиенту
     */
    public function generateTemporaryToken ()
    {
        $token = md5(Yii::app()->params['token_secret'].mktime().rand(0, 1000000));
        $id = $this->getToken();
        $this->_dbDriver->addTemporaryToken($id, $token);
        return $token;
    }

    public function findTokenByTtoken ($ttoken)
    {
        return $this->_dbDriver->findTokenByTtoken($ttoken);
    }
    ...
    {
        return $this->_dbDriver->removeTemporaryToken($token);
    }
