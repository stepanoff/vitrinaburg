                                            ...
                                            ezcImageThumbnailFilters
{
	...
	private function getProfileString() { // добавляем параметр, если картинка CMYK
        return '';
        /*371: Обернул проверку colorspace, чтобы с ошибкой не падал (В.П.)*/
        try
        {
            $imageColorspace = ImageColorspaceDetectHelper::detect(
                $this->getReferenceData($this->getActiveReference(), 'resource')
            );
        }
        catch(Exception $e)
        {
        ...
        if($imageColorspace === 'CMYK') {
			return '-profile '.Yii::app()->getBasePath().'/sRGB_v4_ICC_preference.icc';
        }
    ...
    public function scale( $width, $height, $direction = ezcImageGeometryFilters::SCALE_BOTH )
    {
        ...
        $this->addFilterOption(
            ...
            '-define filter:support=2.5 -filter Lagrange -quality 95 '.$this->getProfileString().' -resize',
    ...
    public function scaleExact( $width, $height )
    {
		...
		// Дмитрий: (42) добавлен -coalesce для корректного ресайза gif анимации
		$data = getimagesize( $this->getActiveResource() );
		// Дмитрий: Наложение watermark на анимированные GIF (42)
    ...
    /**
     * Данный метод это доработанный watermarkPercent:
     * 1. Использует gravity в качестве определения позиции для watermark image
     * 
     * @param string $image  The image file to use as the watermark
     * @param string $gravity Gravity position
     * @param int $posX      X position in the destination image in percent.
     * @param int $posY      Y position in the destination image in percent.
     * @param int|bool $size Percentage size of the watermark, false for none.
     * @return void
     ...
     *         If a submitted parameter was out of range or type.
     */
    public function watermarkCustom( $image, $gravity, $posX = 0, $posY = 0, $size = false )
    {
    	$gravityList = array('NorthWest', 'North', 'NorthEast', 'West', 'Center', 'East', 'SouthWest', 'South', 'SouthEast');
    	...
    	if(!in_array($gravity, $gravityList))
    	{
    		throw new ezcBaseValueException( 'gravity', $gravity, 'not in list'.print_r($gravityList) );
    	}
    	...
    	// Раскоментировать если требуется менять прозрпачность у watermark
//     	$this->addFilterOption(
//             $this->getActiveReference(),
//             '-compose',
//         	'dissolve -define compose:args=100x50'
//         );
        ...
        $this->addFilterOption(
            $this->getActiveReference(),
             '-geometry',
            ( $watermarkWidth !== false ? $watermarkWidth : "" ) . ( $watermarkHeight !== false ? "x$watermarkHeight" : "" ) . $posX.$posY
        );
    ...
    public function watermarkAbsolute( $image, $posX, $posY, $width = false, $height = false )
    {
		...
		// Дмитрий: Наложение watermark на анимированные GIF (42)
		if ($data['mime'] == 'image/gif'){
			$this->watermarkGif($image, $posX, $posY, $width , $height );
			return;
		}
	...
	public function watermarkGif($image, $posX, $posY, $width = false, $height = false){

		if ($width === false || $height === false){
			$waterData = getimagesize($image);
			$width = $waterData[0];
			$height = $waterData[1];
		}

		$command = 'convert '.$image.' -geometry '. $width. 'x'.$height .' -write mpr:wmark +delete '.$this->getActiveResource().' -coalesce -gravity southeast -draw \'image over 10,10 0,0 "mpr:wmark"\' '.$this->getActiveResource();
		shell_exec($command);
	}
